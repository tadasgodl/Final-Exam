...
function validate_fields_match($form_values, array &$form, array $params): bool
{
    foreach ($params as $field_index) {
        if ($form_values[$params[0]] !== $form_values[$field_index]) {
            $form['fields'][$field_index]['error'] = strtr('Passwords do not match', [
...
/**
 * Check that field doesn't contain any numbers
 *
 * @param string $field_value
 * @param array $field
 * @return bool
...
function validate_not_numeric(string $field_value, array &$field): bool {
    if (preg_match( '/\d/', $field_value)) {
        $field['error'] = 'Field cannot contain any numbers';
...
/**
 * Checks if fields has any special characters
 *
 * @param string $field_value
 * @param array $field
 * @return bool
 */
function validate_no_spec_chars(string $field_value, array &$field): bool {
    if (preg_match('/[\'^£$%&*!()}{@#~?><>,|=_+¬-]/', $field_value)) {
        $field['error'] = 'Field cannot contain any special characters';
...
function validate_char_length(string $field_value, array &$field, array $params): bool {
    if (strlen($field_value) < $params['min'] || strlen($field_value) > $params['max']) {
        $field['error'] = strtr('Field must not exceed @max characters!', [
            '@max' => $params['max']
        ]);
